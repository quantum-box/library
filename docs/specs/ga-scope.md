...
| 公開 | `repo.is_public=true` の `/docs/{org}/{repo}`, `/docs/{org}/{repo}/{data_id}`, `/docs/{org}/{repo}/{data_id}/md` による公開閲覧と Markdown 取得 | 公開 UI の細かな表示差分や埋め込み素材の高度な正規化 | 外部 CMS / GitHub / Notion 等との双方向公開同期 |
