...
| 公開 | `repo.is_public=true` の `/docs/{org}/{repo}`, `/docs/{org}/{repo}/{data_id}`, `/docs/{org}/{repo}/{data_id}/md` による公開閲覧と Markdown 取得 | 公開 UI の細かな表示差分や埋め込み素材の高度な正規化 | 外部 CMS / Notion / GitHub 等との双方向公開同期（GitHub one-shot import のみ GA、詳細は §3） |
...
外部連携の詳細な readiness は [外部連携 readiness](integrations/readiness.md) に従う。Library GA では、HubSpot / Stripe / Notion / Airtable の NoOp 外部連携、外部同期、webhook / API pull の実行経路を GA 対象外とする。GitHub は one-shot Markdown import のみを GA とし、push webhook、API pull、Data 保存時の自動 writeback、`syncDataToGithub` / `bulkSyncExtGithub` / `enableGithubSync` による継続同期は Experimental とする（2026-09-11 再判定）。GitHub App installation も GA 対象外のまま。
...
6. `importMarkdownFromGithub` は `enableGithubSync=false` の one-shot import のみを GA とする（デフォルト false）。`enableGithubSync=true`、`syncDataToGithub`、`bulkSyncExtGithub`、`enableGithubSync` mutation、および `ext_github.enabled=true` の Data 保存時自動 writeback は Experimental とする。
