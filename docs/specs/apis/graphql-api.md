...
28. `sync_data_to_github(input: SyncDataToGitHubInput): SyncResult`（Experimental: 単一 Data の legacy GitHub writeback。`dryRun` 対応）
29. `bulk_sync_ext_github(input: BulkSyncExtGitHubInput): SyncResult`（Experimental: repo 内全 Data への legacy `ext_github` 一括設定 + writeback）
30. `enable_github_sync(input: EnableGitHubSyncInput): SyncResult`（Experimental: legacy `ext_github` property を作成）
...
35. `import_markdown_from_github(input: ImportMarkdownFromGitHubInput): ImportMarkdownResult`（GA はデフォルトの `enableGithubSync=false` による one-shot import。`true` の継続同期は Experimental）
