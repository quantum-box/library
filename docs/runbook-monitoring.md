...
| --- | --- | --- | --- |
| API availability | `GET /`, `GET /version`, external synthetic check, Lambda `Errors` / `Throttles` | 1 分 | API が利用可能で、production build が応答していることを確認する |
| API latency | API Gateway latency, Lambda duration p50 / p95 / p99, trace span duration | 5 分 | GA 後の実利用増加で体感速度が劣化していないことを確認する |
...
| 条件 | 判定窓 | 初動 |
| --- | --- | --- |
| API synthetic check が 3 回連続失敗 | 3 分 | `GET /version`、Lambda logs、直近 deploy を確認し、起動 guard / runtime error なら rollback |
| Lambda `Errors` が 5 分で 10 件以上、または 5xx rate が 5% 以上 | 5 分 | Sentry issue と CloudWatch logs を確認し、単一 endpoint か全体障害かを分類 |
...
3. Sentry issue、CloudWatch logs、trace、GitHub deployment / workflow を同じ incident note にリンクする。
...
1. API overview: request count、5xx、4xx、p50 / p95 / p99 latency、Lambda errors / throttles。
