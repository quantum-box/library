...
- Fetch selected records with `get_data` (`org`, `repo`, `data_id`) before summarizing. It returns a title, ID, canonical URL, Markdown body, typed `property_data`, and a string `record_version`. An empty search is not proof that no relevant content exists.
...
- For a workflow with a known stable Data ID, use `upsert_data` to create or update that same record and inspect `outcome`. Retrying with the same ID avoids duplicate records but can still advance the revision or overwrite a concurrent edit. Do not invent IDs where the user intends an update to an existing record.
