...
- Fetch selected records with `get_data` (`org`, `repo`, `data_id`) before summarizing. It returns a title, ID, canonical URL, Markdown body, typed `property_data`, and a string `record_version`. Every write to the record, including MCP `update_data` / `upsert_data`, advances the version, so a different value on a later read means the record changed in between. MCP writes do not take it as an expected version (they are last-writer-wins), so it cannot prevent a concurrent edit from being overwritten. An empty search is not proof that no relevant content exists.
...
- `update_data` patches the supplied properties, so send only properties being changed. Read the existing record before editing content and preserve unrelated content. Preserve the existing title unless renaming was requested: `name` is optional only because a `markdown` document carries its own title. Use the typed `property_data` returned by `get_data` rather than re-deriving values by hand. `value_type` supports all current property kinds, including `id` and `location`; auto-generated Id properties are immutable.
- To rewrite a record's body, or several properties at once, send the Markdown `get_data` returned, edited, as `markdown` on `update_data` / `upsert_data` / `create_data`. Frontmatter keys are property names and the body becomes the record body, so the document round-trips. A key matching no property is reported in `warnings` and skipped rather than failing the write — read them back. An empty body leaves the body property untouched, and an explicit `property_data` entry overrides the document for that property.
