/* @ts-self-types="./photon_engine.d.ts" */
 ...
 * @returns {any}
 */
export function photon_engine_apply_operation(current, operation) {
    const ret = wasm.photon_engine_apply_operation(current, operation);
    if (ret[2]) {
...
/**
 * @param {string | null | undefined} current_json
 * @param {string} operation_json
 * @returns {string}
 */
export function photon_engine_apply_operation_json(current_json, operation_json) {
    let deferred4_0;
    let deferred4_1;
    try {
        var ptr0 = isLikeNone(current_json) ? 0 : passStringToWasm0(current_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        var len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(operation_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.photon_engine_apply_operation_json(ptr0, len0, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('photon_engine_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
