...
const deploymentMode = resolveDeploymentMode(
  viteEnv.VITE_LIBRARY_DEPLOYMENT_MODE ?? viteEnv.VITE_PHOTON_DEPLOYMENT_MODE
)
const syncBackend = resolveSyncBackend(
  deploymentMode,
  viteEnv.VITE_LIBRARY_SYNC_BACKEND ?? viteEnv.VITE_PHOTON_SYNC_BACKEND
)
const frontendWorkerRuntime = resolveFrontendWorkerRuntime(
  deploymentMode,
  viteEnv.VITE_LIBRARY_FRONTEND_WORKER_RUNTIME ?? viteEnv.VITE_PHOTON_FRONTEND_WORKER_RUNTIME
)
const appProfile = {
  id: 'library-client',
...
const defaultTenantWorkspace = normalizeTenantWorkspaceOption(
  {
    tenantId: viteEnv.VITE_LIBRARY_TENANT_ID ?? viteEnv.VITE_PHOTON_TENANT_ID ?? DEFAULT_TENANT_ID,
    tenantName: viteEnv.VITE_LIBRARY_TENANT_NAME ?? viteEnv.VITE_PHOTON_TENANT_NAME ?? 'Library',
    workspaceId: viteEnv.VITE_LIBRARY_WORKSPACE_ID ?? viteEnv.VITE_PHOTON_WORKSPACE_ID ?? DEFAULT_WORKSPACE_ID,
    workspaceName: viteEnv.VITE_LIBRARY_WORKSPACE_NAME ?? viteEnv.VITE_PHOTON_WORKSPACE_NAME ?? 'Library',
    workspaceInitial: viteEnv.VITE_LIBRARY_WORKSPACE_INITIAL ?? viteEnv.VITE_PHOTON_WORKSPACE_INITIAL,
  },
  {
...
const availableTenantWorkspaces = resolveTenantWorkspaceOptions(
  viteEnv.VITE_LIBRARY_TENANT_WORKSPACES ?? viteEnv.VITE_PHOTON_TENANT_WORKSPACES,
...
export const appKitConfig: AppKitConfig = {
  ...
  workspace: {
    ...
    projects: [
      ...
      { id: 'cms-content', label: 'CMS Content' },
    ...
    definitions: [
      {
        ...
        name: 'Default Data Workflow',
        description: 'Default Library data publishing flow.',
        stages: [
          {
            id: 'intake',
            label: 'Intake',
  ...
  chat: {
    ...
    stream: {
      mode: resolveChatStreamMode(
        deploymentMode,
        viteEnv.VITE_LIBRARY_CHAT_STREAM_MODE ?? viteEnv.VITE_PHOTON_CHAT_STREAM_MODE
      ),
      transport: resolveChatStreamTransport(
        viteEnv.VITE_LIBRARY_CHAT_STREAM_TRANSPORT ?? viteEnv.VITE_PHOTON_CHAT_STREAM_TRANSPORT
      ),
      endpoint: chatStreamEndpoint,
      websocketUrl: viteEnv.VITE_LIBRARY_AGENT_WS_URL ?? viteEnv.VITE_PHOTON_AGENT_WS_URL,
      authToken: viteEnv.VITE_LIBRARY_AGENT_AUTH_TOKEN ?? viteEnv.VITE_PHOTON_AGENT_AUTH_TOKEN,
      toolResultPath:
        viteEnv.VITE_LIBRARY_AGENT_TOOL_RESULT_PATH ??
        viteEnv.VITE_PHOTON_AGENT_TOOL_RESULT_PATH ??
  ...
  server: {
    backend: resolveAppServerBackend(
      viteEnv.VITE_LIBRARY_APP_SERVER_BACKEND ?? viteEnv.VITE_PHOTON_APP_SERVER_BACKEND
    ),
