...
function applyClientOperationInMemoryForTests<T>({
  ...
  const nextValue =
    kind === 'patch'
      ? { ...((existing?.value as Record<string, unknown> | undefined) ?? {}), ...(fields ?? {}) }
      : kind === 'delete'
        ? existing?.value ?? {}
        : value ?? {}
  const projected: EngineRuntimeRecord = {
    key: {
      scope: engineScope,
      collection,
      record_id: recordId,
    ...
    value: nextValue,
    version: runtimeTimestamp(nowMs),
    field_versions: {},
    deleted_at: kind === 'delete' ? runtimeTimestamp(nowMs) : null,
    updated_by: engineActorId,
  }
...
export async function syncClientEngineOperations(
  ...
  if (isTestMode) {
    const pending = testOperations.filter((operation) => operation.status === 'pending')
    if (pending.length === 0) {
    ...
    for (const decision of result.decisions ?? []) {
      ...
      const operation = pending.find((entry) => entry.operation.id === decision.operation_id)
