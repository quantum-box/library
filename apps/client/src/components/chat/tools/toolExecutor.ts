...
function requireRecordRuntime(context?: ToolRuntimeContext) {
  ...
  if (!recordTools) {
    throw new Error('Library data tools are not available in this chat context')
  }
  return recordTools
}
...
function recordListMessage(action: RecordToolResponse['action'], records: DatabaseRecord[], total: number) {
  const noun = 'data'
  if (action === 'get') return `Found ${records[0]?.identifier ?? 'data'}`
  if (action === 'create') return `Created ${records[0]?.identifier ?? 'data'}`
  if (action === 'update') return `Updated ${records[0]?.identifier ?? 'data'}`
  if (action === 'move') return `Moved ${records[0]?.identifier ?? 'data'}`
...
registerTool({
  ...
  description: 'Search Library API data from the canonical Library data store',
  icon: 'database',
...
registerTool({
  type: 'record_list',
  name: 'Library Data List',
  description: 'List Library API data from the canonical Library data store',
  icon: 'database',
...
registerTool({
  type: 'record_get',
  name: 'Data Lookup',
  description: 'Get Library data by id or identifier',
  icon: 'record',
...
registerTool({
  type: 'record_create',
  name: 'Create Data',
  description: 'Create Library data through the Library API',
  icon: 'record-plus',
...
registerTool({
  type: 'record_update',
  name: 'Update Data',
  description: 'Update Library data through the Library API',
  icon: 'record-edit',
...
registerTool({
  type: 'record_move',
  name: 'Move Data',
  description: 'Move Library data to another workflow status',
  icon: 'record-move',
