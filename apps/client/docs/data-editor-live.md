...
- モバイルのブラウザは非表示のページを `pagehide` なしで破棄することがあるため、
  ...
  通常のリクエストで送り直す。`keepalive` の保存は、GraphQLの応答を待ってから
  RESTへ切り替える時間が離れるページには無いため、送り先をその場で決める。
  それまでのGraphQLの応答（ページの読み込みなど）で使えると分かっていれば
  GraphQL、無い（404/405/501）と分かっていればREST。まだ分からなければ、RESTで
  表せる値だけならREST、表せない型のプロパティ（Selectなど）があればGraphQLで送る。`keepalive` の保存は、アクセストークンの更新時期に
...
- 付け替えの再描画は編集として扱わない。付け替えるとundo履歴は新しいルームから
  やり直しになるが、参加前の入力をルームへ書き込む処理は、先に作ったルームの
  undo管理のもとで行う（`roomUndo.ts`）。参加前の入力は付け替え後も1回の
  元に戻す操作で取り消せる。
