...
/** Check whether the Database BC owns the value of this Id property. */
function isAutoGeneratedId(property: PropertyForEditorFragment): boolean {
	return (
		property.typ === PropertyType.Id &&
		(property.meta as IdTypeMetaForEditorFragment | undefined)?.autoGenerate ===
			true
	)
}

/** Preserve read-only behavior for repos created before typed Id metadata. */
function isLegacyGeneratedId(property: PropertyForEditorFragment): boolean {
	return property.typ === PropertyType.String && property.name === 'id'
}
...
function isSystemProperty(property: PropertyForEditorFragment): boolean {
	return (
		isAutoGeneratedId(property) ||
		isLegacyGeneratedId(property) ||
		['createdAt', 'updatedAt'].includes(property.name)
	)
}
...
export function PropertyValue({
	...
	// Auto-generated IDs are owned by the Database BC and equal the canonical Data ID.
	const value =
		isAutoGeneratedId(property) || isLegacyGeneratedId(property)
			? dataId
			: getPropertyValue(property, propertyData)
			...
			case PropertyType.Id:
				...
				return (
					<Input
						...
						readOnly
						disabled
						className='h-10 w-full rounded-xl border-border/60 bg-muted/60'
					/>
				)
			case PropertyType.String:
			case PropertyType.Integer:
