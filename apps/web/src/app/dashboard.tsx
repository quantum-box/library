			...
			try {
				...
				if (privateRepoCandidates.length > 0) {
					...
					const repoSelections = privateRepoCandidates
						.map(
							(_, index) =>
								`repoAccess${index}: canViewRepo(orgUsername: $org${index}, repoUsername: $repo${index})`,
						)
						.join('\n')
					...
					try {
						const accessResults = await executeGraphQL<
						...
						for (let index = 0; index < privateRepoCandidates.length; index++) {
							...
							if (accessResults[`repoAccess${index}`] === true) {
