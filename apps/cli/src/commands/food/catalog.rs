...
fn chapter3_remark(
    ...
    if nutrient_key != "ID" || raw.trim() != "*" {
        return None;
    }
    remarks?
        .lines()
        .map(str::trim)
        .find(|l| l.contains("ヨウ素") && l.contains("第3章参照"))
        .map(str::to_string)
}
...
pub struct TargetCatalog {
    ...
    /// Food groups that are missing a matching worksheet.
...
pub fn build_catalog(
    ...
    let mut reported_category_errors = BTreeSet::new();
    ...
    for food in foods {
        ...
        let category_name = match food.group_code.as_deref() {
            Some(group_code) => match opts.categories.get(group_code) {
                Some(name) => Some(name.clone()),
                None => {
                    let key = Some(group_code.to_string());
                    if reported_category_errors.insert(key) {
                        out.category_errors.push(format!(
                            "food group {group_code:?} has no matching worksheet (for example food {})",
                            food.food_code
                        ));
                    }
                    None
            ...
            None => {
                if reported_category_errors.insert(None) {
                    out.category_errors.push(format!(
                        "food {} (row {}) has no food group code",
                        food.food_code, food.row
                    ));
                }
                None
            }
        ...
        for n in &table.layout.nutrients {
            ...
            if let Some(remark) =
                chapter3_remark(&n.key, raw, food.remarks.as_deref())
            {
...
pub(super) mod tests {
    ...
    pub fn options() -> BuildOptions {
        BuildOptions {
            ...
            categories:
                categories_from_sheet_names(&fixtures::table_book())
                    .unwrap(),
        }
    }
    ...
    fn chapter3_stars_are_deferred_not_quarantined() {
        ...
        assert!(!c
            .quarantine
            .iter()
            .any(|q| q.nutrient_key.as_deref() == Some("ID")));
        assert!(!c
            .values
            .iter()
            .any(|v| v.business_key == "mext-10330/ID"));
        // A `*` without the chapter 3 remark is still an anomaly.
        assert_eq!(chapter3_remark("ID", "*", Some("別名： あじ")), None);
        assert_eq!(
            chapter3_remark("NA", "*", Some("ヨウ素： 第3章参照")),
            None
        );
        assert_eq!(
            chapter3_remark(
        ...
        assert_eq!(
            chapter3_remark("ID", "Tr", Some("ヨウ素： 第3章参照")),
            None
        );
    }
