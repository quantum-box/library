...
fn parse_itemized(
    ...
    for r in (h.row + 1)..sheet.height() {
        let food_code = text(sheet, r, Some(h.code));
        let item = text(sheet, r, Some(h.item)).unwrap_or_default();
        let target = text(sheet, r, h.target).unwrap_or_default();
        let wrong = text(sheet, r, Some(h.wrong));
        let right = text(sheet, r, Some(h.right));
        let note = text(sheet, r, h.note);
        let Some(food_code) = food_code else {
            if item.is_empty()
                && target.is_empty()
                && wrong.is_none()
                && right.is_none()
            ...
            return Err(format!(
                "row {} has errata content but no food number",
                r + 1
            ));
        };
        let field =
        ...
        match marker_col(r).as_deref() {
            ...
            Some("正") => {
                let w = pending.take().ok_or_else(|| {
                    format!(
                        "row {} has 正 without a preceding 誤 row",
                        r + 1
                    )
                })?;
