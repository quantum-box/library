...
impl Provider {
    pub fn readiness(&self) -> ProviderReadiness {
        match self {
            ...
            Provider::Github | Provider::Square => {
    ...
    pub fn unavailable_reason(&self) -> Option<&'static str> {
        match self {
            Provider::Github => Some(
                "GitHub continuous sync is experimental until durable webhook dispatch and ChangeSet-based conflict handling are available.",
            ),
...
mod tests {
    ...
    fn test_non_ga_providers_are_not_runtime_available() {
        ...
        assert_eq!(
            Provider::Github.is_runtime_available(),
            std::env::var("LIBRARY_ENABLE_EXPERIMENTAL_INTEGRATIONS")
                .map(|value| {
                    value.eq_ignore_ascii_case("true") || value == "1"
                })
